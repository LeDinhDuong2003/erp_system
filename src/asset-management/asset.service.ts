...
export class AssetService {
  ...
  async findAll(
    ...
    // For search across asset_name and asset_code
    if (search) {
      // TypeORM find doesn't support OR with ILike easily in find, use query builder
      const qb = this.assetRepository.createQueryBuilder('asset')
        .leftJoinAndSelect('asset.category', 'category');

      qb.where('(asset.asset_name ILIKE :s OR asset.asset_code ILIKE :s)', { s: `%${search}%` });

      if (categoryId) qb.andWhere('category.id = :categoryId', { categoryId });
      if (status) qb.andWhere('asset.status = :status', { status });
      qb.orderBy(`asset.${sortBy}`, sortOrder as 'ASC' | 'DESC');
    ...
    // No search: use find with where object
    const qb = this.assetRepository.createQueryBuilder('asset')
      .leftJoinAndSelect('asset.category', 'category');
  ...
  async create(dto: CreateAssetDto) {
    ...
    const exist = await this.assetRepository.findOne({ where: { asset_code: dto.asset_code } });
  ...
  async update(id: number, dto: UpdateAssetDto) {
    const asset = await this.assetRepository.findOne({ where: { id }, relations: ['category'] });
    if (!asset) throw new NotFoundException('Asset not found');
    ...
    if (dto.purchase_date !== undefined) asset.purchase_date = dto.purchase_date;
    ...
    if (dto.current_holder_id !== undefined) asset.current_holder_id = dto.current_holder_id;
  ...
  async remove(id: number) {
    ...
    // Add business rules: do not delete if status = IN_USE, etc. (example)
    if (asset.status === AssetStatus.IN_USE) {
      throw new BadRequestException('Không thể xóa tài sản đang được sử dụng');
    }
