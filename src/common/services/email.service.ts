...
export class EmailService {
    ...
    if (smtpHost && smtpUser && smtpPass) {
      try {
        ...
        await transporter.sendMail({
          from: process.env.SMTP_FROM || smtpUser,
          to: email,
          subject: 'Mã OTP đăng nhập - ERP System',
          html: this.getLoginOTPTemplate(fullName, otp),
        });

        this.logger.log(`✅ Login OTP sent successfully to ${email}`);
        return;
      } catch (error: any) {
        this.logger.error(`❌ Failed to send login OTP email via SMTP: ${error.message}`);
        this.logger.warn(`⚠️  Falling back to console logging.`);
      }
    }

    // Fallback: Log to console
    this.logger.warn(`📧 [LOGIN OTP - DEVELOPMENT MODE]`);
    this.logger.warn(`   To: ${email}`);
    this.logger.warn(`   Subject: Mã OTP đăng nhập - ERP System`);
    this.logger.warn(`   OTP: ${otp}`);
    this.logger.warn(`   ⚠️  OTP expires in 10 minutes`);
  }
   ...
   */
  private getLoginOTPTemplate(fullName: string, otp: string): string {
    return `
      <!DOCTYPE html>
      <html lang="vi">
      <head>
      ...
      <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
        <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 30px; text-align: center; border-radius: 10px 10px 0 0;">
          <h1 style="color: white; margin: 0;">Mã OTP đăng nhập</h1>
        </div>
        <div style="background: #f9f9f9; padding: 30px; border-radius: 0 0 10px 10px;">
          <p>Xin chào <strong>${fullName}</strong>,</p>
          <p>Bạn đã yêu cầu đăng nhập với xác thực 2 yếu tố. Vui lòng sử dụng mã OTP sau để hoàn tất đăng nhập:</p>
          ...
          <p style="color: #d32f2f; font-size: 14px; margin-top: 20px;"><strong>⚠️ Lưu ý:</strong> Nếu bạn không yêu cầu đăng nhập, vui lòng bỏ qua email này và thay đổi mật khẩu ngay lập tức.</p>
